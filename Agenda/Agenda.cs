...
namespace Agenda
{
    class Agenda
    {
        ...
        public Contacto Buscar(int telefono)
        {
            Contacto aux;
            aux = inicio;
            while (aux != null && aux.Telefono != telefono) // Mientras no sea null el contacto y sea diferente el telefono al buscado pasaremos de siguiente en siguiente
            {                                               // En caso de encontrar el numero, el ultimo contacto en aux sera el contacto buscado
                aux = aux.Siguente;
            }
            if (aux != null) // Si lo encuentra aux, no sera null, entonces regresa el contacto buscado
                return aux;
            else
                return null;
        }
